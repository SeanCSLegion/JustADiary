...
enum BackupService {
    static func exportBackup(includeSettings: Bool) async throws -> URL {
        ...
        let blockRows = repo.rawBlocksForBackup()
        ...
        let diaryCount = repo.countDiaries()
        let blockCount = repo.countBlocks()
        ...
        let outName = "JustDiary_\(formatter.string(from: stampDate)).jdiary"
    ...
    static func importBackup(fileURL: URL, mode: String) async throws -> BackupStats {
        ...
        try repo.mergeBackupDatabase(path: dbPath, mode: mode, copyImage: copyImage, stats: &stats)
