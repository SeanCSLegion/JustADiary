...
- `LanguageThemeUITests` 覆盖设置变更，回归有保障（当时提到的 `ImportUITests` 覆盖备份导入导出，后来因为整份测试没有任何断言、永远通过，已被删除）；
...
**收益**：包体积 **−2.9 MB（−94%）**；代码 **−约 1000 行**；全球可用；完全离线；不再有固定尺寸假设（手机竖屏 / 横屏都能直接排）。
**代价**：失去"地图"的视觉呈现。
...
> 编辑器格式栏与首页胶囊。编辑页的字体模型与遗留问题处理结果见 `docs/editor-typography.md`。
