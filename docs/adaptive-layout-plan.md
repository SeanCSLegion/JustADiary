...
5. **月视图是一条连续滚动的月份流**（竖屏整屏与横屏左栏同一套）：大月份标题 + 星期栏
   钉在顶部、标题跟着滚动实时切换，下面的月份无级滚动；月份之间只隔「小标题这一行字」
   （竖屏 21pt / 横屏 18pt），小标题（「10月」）站在**当月 1 号那一列的正上方**、
...
```
┌ 九月 ◀── 大标题：钉住；内容 = 占住视口顶部的那一个月，滚动时实时切换
│ 一 二 三 四 五 六 日 ◀── 星期栏：钉住
├──────────────────────────────────────────────
│ 28 29 30                  ← 上个月的最后一行（相邻月的日期不画，留白）
│                 10月       ← 小标题带 = 这一行字的高度（竖屏 21pt），
...
3. **静止位置 = 该月第一行的顶**：进入某个月（初始 / 今天 / 年历点月）时把第一行顶到
   ...
   与月↔周 morph 的起点都不用重新推导。五行的月份底下会露出下个月的小标题带
