...
> **当前状态（2026-09-16）**：手机端**已实现**（首页横屏分栏、日历密度降级、横屏左右
> 滑动翻月、足迹/搜索/设置/阅读页的横屏版面、关键词栏），并有 `AdaptiveLayoutTests`
> 与 `LandscapeLayoutUITests` 守住。iPad / Mac 的三栏版面仍是「骨架先行版」
> （`docs/design/landscape/wide.html`），按你的要求后续单独打磨。
>
> 三个已确认的前提：
> 1. **竖屏交互完全不动** —— 年历 / 月历 / 周历三态与之间的 morph 动画原样保留。
> 2. **横屏不提供年份切换** —— 只上下滑切月；横屏不引入第二套交互，以免和竖屏的年历
>    morph 打架。
> 3. **横屏左侧要让出 62pt** —— 系统浮条在横屏变成贴左边缘的竖排胶囊，灵动岛也在
>    左侧同一条竖带上。实测 `safeAreaInsets`：竖屏 `T62 L0 B34 R0`、横屏 `T0 L62 B20 R62`，
>    所以横屏可用内容宽度是 **750pt**（874 − 62 − 62）。
...
重新排版，并把内容右移 `safeArea.leading`（§5.2 的 `AdaptiveLayout.contentInset`）。
...
这正是实现里 `AdaptiveLayout.contentInset` 的用途，`MonthPane` 已经据此右移。
...
| 形态 | 版面 |
|---|---|
| 竖屏（402pt） | **保持现状**：`‹ 年月` 胶囊 +「今天」→ 32pt 月标题 → 30pt 星期栏 → 六行月格（有日记＝下划线、今天＝圆环、选中＝实心圆、未来 35%）；点日期 morph 到周视图、点年月胶囊 morph 到年历。设计稿里这三屏（`ph-home-portrait` / `ph-home-week` / `ph-home-year`）是**按实现 1:1 复刻**的对照基准 |
| 横屏（874 × 402） | 左侧 62pt 让给系统 → **可用 750pt**；**左栏月历 345pt + 右栏日记**；左栏上下翻月 |
| 横屏・高度不足 | 左栏降级为**周条**（一行 7 天，横向翻周），右栏高度不变 |
| 横屏・年份 | **不提供年份切换**，上下滑跨月时自然跨年；「今天」放在左栏标题行 |
| iPad / Mac | 三栏：导航轨 + 中栏 + 右栏（先行版，后续打磨） |

**日历密度按高度自适应**，这是替代原来「月↔周整屏 morph」的关键：

```
可用高度 ≥ 6×56 + 标题      → 月格（6 行，显示农历）
可用高度 ≥ 6×44 + 标题      → 月格（隐藏农历行）
否则                        → 周条（1 行，横向翻周）
```

横屏手机 402pt 高 → 月格每格 47pt（日号 13pt + 农历 9.5pt），是能读的下限；
...
- 横屏真正约束内容边界的是**左侧导航胶囊（12–76）**，不是岛 —— 岛只占 x 0–37。
- 标题行与月格都从 92pt 起（胶囊右边 76 + 16 余量），两处一致，不再各写一个数。
- 这套数值来源：真机 UI 测试探针（导航胶囊 x 12–76、y 338–402）。**实现时不要写死**：
  `contentInset / gridInset / headInset` 都要从 `safeAreaInsets.leading` 派生，这样
  Duo 的折痕、左右不对称的安全区、未来 27.1 的 `reservedRegion` 都能直接接上。


- 导航胶囊与灵动岛**都在左边缘的中间竖带上**，所以横屏没有「左上角一整块不能用」——
  只有 x < 100 这一条竖带要避开。
- 标题行整行从 152pt 起，是因为标题文字恰好落在岛的纵向上；月格从 100pt 起，
  因为只有第 6 行才与岛同高，而第 6 行在 5 周月份里不存在。
- 这套数值来源：真机 UI 测试探针（导航胶囊 `x 12–76`、`y 338–402`）+ 横屏截图目测
  灵动岛位置。实现时这些不是常量，而是 `safeAreaInsets` + 折痕/遮挡 API 的兜底。

### 3.2 足迹 `ph-footprint-landscape.png`

- 手机竖屏：统计行 → 趋势图 → 地点清单，单栏滚动（保持实现）。
- **手机横屏：单列纵向滚动，但「年度趋势图」与「地点清单」并排成两块**（图表 flex:1、
  清单固定 280–300pt），两块同为 190pt 高。
...
- 手机横屏（可用 750pt < 800 分栏阈值）：**条件压成一行**（搜索框 + 时间 + 地点），
...
8. **四屏宽度不统一** —— 每屏各写一套 `padding(.leading, 16 + contentInset)`。现在统一走
   `.adaptivePagePadding()`（左侧 = 系统占位 + 16，右侧 = 16）。
